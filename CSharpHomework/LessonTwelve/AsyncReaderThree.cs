...
class AsyncReaderThree
{
    public static async Task Execute()
    {
        ...
        string projectDirectory = Directory.GetParent(binPath)?.Parent?.Parent?.Parent?.FullName ?? string.Empty;

        string filePath = "MyFiles";

        string directoryPath = Path.Combine(projectDirectory, filePath);

        if (string.IsNullOrEmpty(projectDirectory) || !Directory.Exists(directoryPath))
        {
